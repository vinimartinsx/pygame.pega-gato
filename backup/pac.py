...
# Estado do jogo
def salvar_jogo():
    # Obtém a data e hora atual para registrar o tempo do jogo
    data_hora = time.strftime("%Y-%m-%d %H:%M:%S")
    
    with open('mapa.txt', 'a') as arquivo:
        arquivo.write(f'\nPontuação: {pontuacao} - Tempo: {data_hora}')
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_v:  # Se pressionar a tecla "V"
                    return  # Retorna para a função que a chamou (o menu)

        # Exibir texto do tutorial
        pos_y = 50
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_UP:
                    ...
                    if opcao_selecionada == 0:  # Começar
                        # Lógica para iniciar o jogo
                        menu_ativo = False
                        # Iniciar o jogo aqui
                    elif opcao_selecionada == 1:  # Tutorial
                        mostrar_tutorial()
                        return menu()
                        
                        # Exibir o tutorial e retornar ao menu original
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_UP:
                    ...
                    if opcao_selecionada == 0:  # Recomeçar
                        menu_ativo = False  # Sai do loop do menu
                        # Reiniciar variáveis e pontos coletáveis
...
def verificar_vitoria():
    ...
    if pontuacao == 225:
        ...
        tela_vitoria()
        menu()  # Retorna ao menu inicial após a vitória
...
def gerar_itens():
    ...
    if not pontos_disponiveis:  # Verifica se não há mais pontos disponíveis
        # Aqui você pode reiniciar o mapa ou realizar outra ação quando todos os pontos forem coletados
        # Por exemplo:
        print("Todos os pontos foram coletados!")
        # Aqui você reiniciaria o mapa ou faria algo apropriado para o seu jogo
        return None
...
def jogo():
    ...
    while True:
        ...
        if verificar_vitoria():
            usos_superpoder = 3
            # Coloque isso no local certo onde o jogo verifica a vitória
            break 
        ...
        if verificar_colisao_fantasma():
            if superpoder_ativo:
                ...
                menu2()  # Chama o menu de perda apenas quando há colisão com os fantasmas
                salvar_jogo()

        # Verificar e atualizar o superpoder
        if teclas[pygame.K_s]:
            ativar_superpoder()

        # Verificar a duração do superpoder
        if superpoder_ativo:
